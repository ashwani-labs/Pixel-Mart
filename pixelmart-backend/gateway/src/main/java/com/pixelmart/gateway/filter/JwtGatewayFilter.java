...
public class JwtGatewayFilter implements GlobalFilter, Ordered {
  ...
  private static final List<String> PUBLIC_POST_PATHS =
      List.of(
          "/api/auth/register",
          "/api/auth/login",
  ...
  private ServerHttpRequest enrichRequest(ServerHttpRequest request, String authHeader) {
    ...
    return request.mutate().header("X-User-Id", userId).header("X-Roles", roles).build();
  }

  private ServerWebExchange maybeEnrichWithUser(
      ServerWebExchange exchange, ServerHttpRequest request) {
    String authHeader = request.getHeaders().getFirst(HttpHeaders.AUTHORIZATION);
    if (authHeader == null || !authHeader.startsWith("Bearer ")) {
      return exchange;
    }
  ...
  private boolean isPublic(String path, HttpMethod method) {
    ...
    if (method == HttpMethod.POST && path.equals("/api/orders/guest-checkout")) {
      return true;
    }
