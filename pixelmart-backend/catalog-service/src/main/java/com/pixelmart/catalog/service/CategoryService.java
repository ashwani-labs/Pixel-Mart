...
public class CategoryService {
    ...
    public List<CategoryResponse> listPublic() {
        return categoryRepository.findByParentIdIsNotNullAndActiveTrueOrderBySortOrderAscNameAsc().stream()
                .map(CategoryResponse::from)
                .toList();
    }
    ...
    void validateProductCategory(String categoryId) {
        ...
        if (category.getParentId() == null || category.getParentId().isBlank()) {
            throw new BadRequestException("Products must be assigned to a category under a super category");
        }
    }
    ...
    private void validateHierarchy(String categoryId, String parentId) {
        ...
        if (normalizedParent == null) {
            if (categoryId != null && categoryRepository.countByParentId(categoryId) > 0) {
                throw new BadRequestException("Super categories with sub-categories cannot be nested under another category");
            }
            return;
        }
