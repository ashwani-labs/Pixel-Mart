...
public class OfferService {
  ...
  public OfferPricing price(Product product, String couponCode) {
    ...
    List<Offer> offers =
        offerRepository.findActiveProductOffers(
            product.getId(), product.getCategoryId(), normalizedCoupon, Instant.now());
  ...
  public CartOfferPricing cartDiscount(BigDecimal subtotal, String couponCode) {
    ...
    if (best == null) {
      return new CartOfferPricing(
          BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP), null, null, couponMatched);
    }
    ...
    return new CartOfferPricing(discountTotal, best.getName(), best.getCouponCode(), couponMatched);
  }
  ...
  private BigDecimal cartDiscountAmount(BigDecimal subtotal, Offer offer) {
    BigDecimal discount =
        switch (offer.getType()) {
          case PERCENT ->
              subtotal.multiply(offer.getValue()).divide(ONE_HUNDRED, 2, RoundingMode.HALF_UP);
  ...
  private OfferPricing bestPrice(Product product, List<Offer> offers, boolean couponMatched) {
    ...
    BigDecimal compareAtPrice =
        product.getCompareAtPrice() != null && product.getCompareAtPrice().compareTo(basePrice) > 0
            ? product.getCompareAtPrice()
            : basePrice;
    return new OfferPricing(
        best.price(), compareAtPrice, best.offer().getName(), appliedCoupon, couponMatched);
  }
