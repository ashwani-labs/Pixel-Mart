...
class OrderIntegrationTest {
  ...
  @Autowired private PincodeCacheRepository pincodeCacheRepository;
  ...
  void pincodeLookupReturnsDelhiDataFor110001() throws Exception {
    mockMvc
        .perform(
            get("/api/orders/addresses/pincode/110001")
  ...
  void checkoutWithoutAuthReturns401() throws Exception {
    Map<String, String> body =
        Map.of("addressId", UUID.randomUUID().toString(), "paymentMethod", "MOCK_CARD");
  ...
  void checkoutSnapshotsProductPricesAtOrderTime() throws Exception {
    ...
    Map<String, String> body =
        Map.of(
            "addressId", addressRepository.findAll().get(0).getId(), "paymentMethod", "MOCK_CARD");
  ...
  private void seedCartAndAddress() {
    ...
    addressRepository
        .findByUserIdOrderByDefaultAddressDescCreatedAtDesc(USER_ID)
  ...
  private void stubCatalogClients() {
    CatalogProductSnapshot product =
        new CatalogProductSnapshot(
            ...
            null,
            null);
    when(catalogClient.getProductForCart(eq(PRODUCT_ID), any(), any())).thenReturn(product);
    when(catalogClient.getStoreSettings())
        .thenReturn(new CatalogStoreSettings(true, new BigDecimal("18.00"), "GST", "INR"));
    when(catalogClient.getCartDiscount(any(), any()))
        .thenReturn(
            new CatalogCartDiscountSnapshot(BigDecimal.ZERO.setScale(2), null, null, false));
    doNothing().when(catalogClient).reserveStock(any());
    when(authClient.getUser(USER_ID))
        .thenReturn(new AuthUserSnapshot(USER_ID, "customer@pixelmart.local", "Test Customer"));
