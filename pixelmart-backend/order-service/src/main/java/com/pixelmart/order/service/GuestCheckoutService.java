...
public class GuestCheckoutService {
  ...
  public GuestCheckoutService(
      AuthClient authClient, AddressService addressService, CheckoutService checkoutService) {
  ...
  public GuestCheckoutResponse checkout(GuestCheckoutRequest request) {
    GuestSessionResponse session =
        authClient.createGuestSession(request.email(), request.fullName());
    UpsertAddressRequest addressRequest =
        new UpsertAddressRequest(
            "Checkout",
            request.fullName(),
            request.phone(),
    ...
    OrderResponse order =
        checkoutService.checkoutForUser(
            session.userId(),
            address,
            request.items(),
