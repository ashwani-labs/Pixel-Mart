...
public class PincodeService {
  ...
  private PincodeLookupResponse fetchAndCache(String pincode) {
    ...
    try {
      ...
      List<PostalPincodeApiResponse> body =
  ...
  /**
   * api.postalpincode.in serves an expired TLS certificate and closes Java HTTP clients without a
   * response; curl with insecure TLS + HTTP/1.1 is the reliable transport here.
   */
  private String fetchFromApi(String url) throws IOException, InterruptedException {
    ProcessBuilder processBuilder =
        new ProcessBuilder(
            "curl", "-sk", "--http1.1", "-H", "Accept: application/json", "--max-time", "10", url);
  ...
  private PincodeLookupResponse toLookup(
      String pincode, List<PostalPincodeApiResponse.PostOffice> offices) {
    PostalPincodeApiResponse.PostOffice first = offices.get(0);
    List<PostOfficeOption> options =
        offices.stream()
            .map(
                o ->
                    new PostOfficeOption(
                        o.Name(), o.BranchType(), o.District(), o.Block(), o.State()))
            .toList();
